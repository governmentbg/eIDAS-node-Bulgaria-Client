...
defaultHostname="https://eidas-test.egov.bg"
...
#test env
keytool -genkeypair -keyalg EC -keystore $keystoreFileName -keysize 384 -alias metadata -dname "CN=SP-metadata-signing, OU=test, O=test, C=BG" -validity 730 -storepass $password -keypass $password || { echo 'Could not generate the keystore' ; exit 1; }
...
keytool -genkeypair -keyalg EC -keystore $keystoreFileName -keysize 384 -alias requestsigning -dname "CN=SP-auth-request-signing, OU=test, O=test, C=BG" -validity 730 -storepass $password -keypass $password || { echo 'Could not generate the keypair for signing' ; exit 1; }
keytool -genkeypair -keyalg RSA -keystore $keystoreFileName -keysize 4096 -alias responseencryption -dname "CN=SP-response-encryption, OU=test, O=test, C=BG" -validity 730 -storepass $password -keypass $password || { echo 'Could not generate the keypair for encryption' ; exit 1; }
keytool -importcert -keystore $keystoreFileName -storepass $password -file ../../src/test/resources/scripts/test.pem -alias idpmetadata -noprompt || { echo 'Could not import the trust anchor' ; exit 1; }
...
echo "eidas.client.idp-metadata-url = https://eidas-test.egov.bg/SpecificConnector/ConnectorResponderMetadata">>$confFileName
